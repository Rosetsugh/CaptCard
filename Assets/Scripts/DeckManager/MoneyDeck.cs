...
[ExecuteInEditMode]
public class MoneyDeck : MonoBehaviour
{
    ...
    {
        CardMoney cards = DB_Card.GetComponent<CardMoney>();
    ...
    public void FillSmallDeck()
    {
        ...
        while (counter < cardsInBasicDeck)
        {
            ...
            {
                CardMoney cardMoney = DB_Card.GetComponent<CardMoney>();
                while (true)
                {
                    int randomCardIndex = Random.Range(0, cardMoney.Rows.Count);
                    //if selected cards qType == chosen topic then add it to list
                    if (cardMoney.Rows[randomCardIndex]._Question_Type == Topics[randomTopicIndex])
                    {
                        basicCards.Add(CreateChosenCardBy(cardMoney, randomCardIndex));
                        ChosenTopics.Add(cardMoney.Rows[randomCardIndex]._Question_Type);
                        counter++;
                        break;
                    }
                }
    ...
    private CardTemplate CreateChosenCardBy(CardMoney cardSheet, int index)
    {
